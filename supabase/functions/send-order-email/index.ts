...
serve(async (req) => {
  ...
  try {
    ...
    const productList = products
      .map((p: { name: string; quantity: number }) => `• ${escapeHtml(p.name)} (x${p.quantity})`)
      .join("<br/>");
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        subject: `New Order Request from ${escapeHtml(customer_name)}`,
        html: `
          <h2>New Order Request from PokéMarket!</h2>
          <p><strong>Customer:</strong> ${escapeHtml(customer_name)}</p>
